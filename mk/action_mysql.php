...
//---------------------------------- 리스팅
	...
	$int_id=intval($_GET['int_id']);
	
	$rows='';
	if(!empty($unique_id) && !empty($lists_type)){
		$conn=mysql_connect("localhost","root","sulung2");
		mysql_select_db("mkadd");
		$where="usage_yn='Y' and unique_id='".$unique_id."' and ";
		if(!empty($int_id)) $where.=" _id<".$int_id." and ";
		$where=substr($where,0,-4);
		$query="select _id, user_id, user_name, user_pic, text, regdate, good, bad, hometown from mktable where ".$where." limit 0,".$lists_maxlist;
		...
		while($row=mysql_fetch_array($result)){
			...
			foreach($row as $key=>$val){
				if(!is_numeric($key)){
					$new_row[$key]=htmlspecialchars(trim($val));
				}
			}
			$rows[$idx++]=$new_row;
		}
		$query="select count(*) cnt from mktable where ".$where;
		$result=mysql_query($query);
		$row=mysql_fetch_array($result);
		$total_no=$row['cnt'];
		$resultData=array();
		$resultData['lists']=$rows;
		$resultData['total_no']=$total_no;
		$resultData['lists_last_regdate']=@$list[(count($list)-1)]['_id'];

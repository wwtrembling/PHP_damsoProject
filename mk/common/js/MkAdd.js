var MkAdd={
	...
	init:function(title, unique_id, app_id, cat_01, cat_02, from_url){/*----------------------------------------------------- 시작1*/
		...
		/*로그인 확인*/
		$.getJSON(this.touch_url+"action.php?cmd=check_login&callback=?",function(data){
			...
			if(hometown_obj!=null) {
				...
				$("#mkadd_alllogout_btn").click(function(){
					if(confirm('전체 로그아웃을 하시겠습니까?')) {
						//window.open(MkAdd.touch_url+"action.php?cmd=action_alllogout",'mkpop');
						var mkpop= window.open('','mkpop');
						mkpop.location.href=MkAdd.touch_url+"action.php?cmd=action_alllogout";
					}
				});
			}
		});
	...
	initSub:function(obj){
		...
		if(_check_login ==1){	/*로그인상태*/
			...
			$("#mkadd_"+_type+"_sub_chief_btn").click(function(){
				MkAdd.setCommon(obj);
				$("#mkadd_"+_type+"_sub_div").toggle('fast');
				$.getJSON(MkAdd.touch_url+"action.php?cmd=action_modify_commontype&hometown="+_type+"&callback=?",function(data){});
			});
			/*전송중지버튼*/
				...
				if(MkAdd.result[obj.idx].usage=='Y'){
					if(confirm(_type+'로 전송을 중지하시겠습니까?')) {
						...
						if(MkAdd.result[obj.idx].type==MkAdd.hometown){
							...
							/*서버로 전송하기*/
							$.getJSON(MkAdd.touch_url+"action.php?cmd=action_modify_usage&type="+MkAdd.result[obj.idx].type+"&usage="+MkAdd.result[obj.idx].usage+"&callback=?",function(data){
								/*이미지교체*/
	...
	registAdd:function(str){	/*----------------------------------------------------- 등록하기!!!!!!!!!!!!*/
		...
		$.getJSON(this.touch_url+"action.php?cmd=action_regist&cat_01="+encodeURIComponent(this.cat_01)+"&cat_02="+encodeURIComponent(this.cat_02)+"&title="+encodeURIComponent(this.title)+"&unique_id="+this.unique_id+"&app_id="+this.app_id+"&type="+type+"&str="+encodeURIComponent(str)+"&from_url="+encodeURIComponent(this.from_url)+"&callback=?",
			function(data){
	...
	removeAdd:function(_id){	/*----------------------------------------------------- 삭제!!!!!!!!!!!!*/
		$.getJSON(this.touch_url+"action.php?cmd=action_remove&_id="+_id+"&callback=?",
			function(data){
	...
	loadAdd:function(flag){/*----------------------------------------------------- 불러오기!!!!!!!!!!!!*/
		...
		$.getJSON(this.touch_url+"action.php?cmd=action_list&lists_type="+this.lists_type+"&lists_maxlist="+this.lists_maxlist+"&lists_last_regdate="+this.lists_last_regdate+"&unique_id="+encodeURIComponent(this.unique_id)+"&callback=?",
			function(data){
				if(flag=='new'){/*초기화 로딩*/
					$("#mkadd_div2").html("");
				}
	...
	bindEventTimeline:function(_id){	/*타임라인 이벤트 붙이기*/
		...
		$("#mkadd_"+_id+"_good_btn").click(function(){	/*추천*/
			$.getJSON(MkAdd.touch_url+"action.php?cmd=action_update_click&_id="+_id+"&_clicked_type=g&callback=?",
				function(data){
					/*해당데이타 업데이트*/
					if(data.flag>0){
		...
		$("#mkadd_"+_id+"_bad_btn").click(function(){
			$.getJSON(MkAdd.touch_url+"action.php?cmd=action_update_click&_id="+_id+"&_clicked_type=b&callback=?",
				function(data){
					if(data.flag>0){
						var total=data.total;
		...
		$("#mkadd_"+_id+"_bad2_btn").click(function(){
			$.getJSON(MkAdd.touch_url+"action.php?cmd=action_update_click&_id="+_id+"&_clicked_type=b2&callback=?",
				function(data){
					if(data.flag<0){
